...
export function initArrecadacaoModule(
  ...
  function renderKanban() {
    ...
    const pendingGroups = filterGroupsByFinancial(groupItemsForTable(items)).length;
    els.summary.textContent =
      financialFilter === 'pendentes'
        ? pendingGroups > 0
          ? `${pendingGroups} lead(s) com saldo em aberto no kanban`
  ...
  function renderTable() {
    ...
    const allGroups = groupItemsForTable(items);
    ...
    els.summary.textContent =
      mergedCount > 0
        ? `${groups.length} linha(s) · ${items.length} registro(s) (${mergedCount} agrupada(s))${pendingHint}`
        : financialFilter === 'pendentes'
          ? `${groups.length} lead(s) com saldo em aberto`
